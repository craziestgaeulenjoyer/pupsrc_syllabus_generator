...
export default function PdfViewer({ file }: any) {
  ...
  const isHtmlMode = !!file?.syllabusHtml; // true = render HTML, false = render PDF

  // Scroll tracker (PDF mode only)
  useEffect(() => {
  ...
  // Download: for HTML mode generate a Blob and trigger download
  ...
  return (
    ...
    >
      ...
      {/* ── LEFT SIDEBAR ── */}
      <div className={`hidden lg:flex w-64 border-r flex-col shadow-sm transition-colors duration-300 ${
        ...
        <div className={`h-16 flex items-center px-4 border-b shrink-0 ${isDarkMode ? "border-gray-800" : "border-gray-200"}`}>
          <div className="flex items-center gap-2">
            <Files className={`w-4 h-4 ${isDarkMode ? "text-white" : "text-black"}`} />
        ...
        <div className={`flex-1 overflow-y-auto p-4 space-y-6 ${isDarkMode ? "bg-[#181818]" : "bg-gray-50"}`}>
          {isHtmlMode ? (
            // HTML mode: show section links instead of page thumbnails
            ...
            <Document file={file.url} onLoadSuccess={onDocumentLoadSuccess}>
              {numPages && Array.from(new Array(numPages), (_, i) => (
                <div key={`thumb_${i + 1}`} className="flex flex-col items-center">
                  <div
                    onClick={() => scrollToPage(i + 1)}
                    className={`cursor-pointer transition-all border-4 rounded-sm ${
                      pageNumber === i + 1
                        ? "border-blue-500 shadow-md scale-105"
                        : isDarkMode ? "border-transparent hover:border-gray-700" : "border-white hover:border-gray-300 shadow-sm"
                    }`}
                  >
                    <Page pageNumber={i + 1} width={150} renderTextLayer={false} renderAnnotationLayer={false} />
                  </div>
                  <p className={`text-[11px] mt-2 font-bold ${pageNumber === i + 1 ? "text-blue-600" : "text-gray-400"}`}>
                    {i + 1}
                  </p>
                </div>
              ))}
      ...
      <div className="flex-1 flex flex-col min-w-0 relative">
        {/* Header bar */}
        <header className={`h-16 border-b flex items-center justify-between px-4 sm:px-8 z-10 shrink-0 transition-colors duration-300 ${
          isDarkMode ? "bg-[#1e1e1e] border-gray-800" : "bg-white border-gray-200"
        }`}>
          <button
          ...
          >
            <CircleArrowLeft className="w-5 h-5 transition-all duration-300 group-hover:scale-110 group-hover:-rotate-180 group-hover:text-black" />
            <span className="hidden sm:inline hover:text-[#800000]">Go Back</span>
          </button>
          <div className="flex items-center gap-3">
            <span className={`text-sm font-bold truncate max-w-[150px] sm:max-w-[300px] ${isDarkMode ? "text-white" : "text-black"}`}>
              {file.name}{isHtmlMode ? "" : ".pdf"}
            </span>
          </div>
          <button
        ...
        >
          {isHtmlMode ? (
            ...
            <div className="flex justify-center">
              ...
              >
                <iframe
                  ...
                  onLoad={(e) => {
                    // Auto-expand iframe height to match content
            ...
            <div className="flex flex-col items-center gap-8">
              {loadingError ? (
                <div className={`flex flex-col items-center justify-center p-10 rounded-xl shadow-lg border ${
                  ...
                  <button onClick={() => window.location.reload()} className="mt-4 text-blue-600 text-sm underline font-semibold">
                    Try Again
                  </button>
                </div>
              ) : (
                ...
                >
                  {numPages && Array.from(new Array(numPages), (_, i) => (
                    <div
                      ...
                      className={`shadow-2xl mb-8 ${isDarkMode ? "bg-[#1e1e1e] shadow-black/40" : "bg-white"}`}
                    >
                      <Page
                        pageNumber={i + 1}
        ...
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex items-center gap-2 bg-[#c7c7c7]/60 backdrop-blur-sm border border-gray-800 px-4 py-2 rounded-full shadow-2xl z-20">
          ...
          >
            {isDarkMode
              ? <img src="https://img.icons8.com/?size=100&id=45475&format=png&color=000000" className="w-4 h-4" alt="light" />
              : <img src="https://img.icons8.com/?size=100&id=45474&format=png&color=000000" className="w-4 h-4" alt="dark" />
            }
          </button>
          <button onClick={handlePrint} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-100">
      ...
      {/* ── RIGHT SIDEBAR ── */}
      <div className={`hidden xl:flex w-72 border-l flex-col shadow-sm shrink-0 transition-colors duration-300 ${
        isDarkMode ? "bg-[#1e1e1e] border-gray-800" : "bg-white border-gray-200"
      }`}>
        ...
        <div className="flex-1 p-8 space-y-8 overflow-y-auto">
          <div className={`flex flex-col items-center p-6 rounded-2xl border transition-colors duration-300 ${
            ...
            <div className={`w-16 h-16 rounded-xl shadow-sm flex items-center justify-center mb-4 ${isDarkMode ? "bg-[#252525]" : "bg-white"}`}>
              <img src="https://img.icons8.com/?size=100&id=mcyAsTDJNTI9&format=png" className="w-10" alt="file" />
            </div>
...
// Simple section nav for HTML mode left sidebar
function SyllabusNavLinks({ isDarkMode, iframeRef }: { isDarkMode: boolean; iframeRef: React.RefObject<HTMLIFrameElement> }) {
  const sections = [
