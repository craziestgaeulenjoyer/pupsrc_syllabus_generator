...
const Step2 = () => {
    ...
    const validateData = () => {
        const hasIloMapping = Object.values(iloMapping).some(val => val === true);
        const hasPloMapping = Object.values(ploMapping).some(val => val !== null && val !== '');

        if (!hasIloMapping || !hasPloMapping) {
            setValidationError("Please map at least one outcome in both tables before proceeding.");
    ...
    const validateAndNext = () => {
        if (validateData()) window.location.href = "/syllabus-generator/step-3"; 
    };
        ...
        <div className="min-h-screen bg-[#F3F4F6] flex flex-col font-sans pb-40">
                                            ...
                                            <td className="p-4 min-w-75">
                                                <div className="flex gap-3">
                                                    ...
                                                    <textarea 
                                                        ...
                                                        className="w-full text-sm p-3 bg-white border border-slate-200 rounded-lg focus:ring-2 focus:ring-red-100 focus:border-[#800000] transition-all resize-none shadow-sm min-h-15" 
                                                        value={plo.label} 
                                                        onChange={(e) => {
                                                            const newPlos = [...plos];
                                        ...
                                        <tr key={clo.id} className="hover:bg-slate-50/40 transition-colors group">
                                            <td className="p-4 min-w-87.5">
                                                <textarea 
                                                    ...
                                                    className="w-full text-sm p-3 bg-white border border-slate-200 rounded-lg focus:ring-2 focus:ring-red-100 focus:border-[#800000] transition-all resize-none shadow-sm" 
                                                    value={clo.text} 
                                                    onChange={(e) => {
                                                        const newClos = [...clos];
                                            ...
                                            {plos.map((plo) => (
                                                <td key={plo.id} className="p-1.5 text-center">
                                                    <select 
                                                        ...
                                                        value={ploMapping[`${clo.id}-${plo.id}`] || ''}
            ...
            <footer className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 p-3 sm:p-4 z-40 shadow-[0_-4px_20px_-5px_rgba(0,0,0,0.1)]">
                <div className="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-center gap-3">
                    <div className={`flex items-center gap-2 text-[10px] sm:text-xs font-bold px-4 py-2 rounded-full ${isMapped ? 'bg-green-50 text-green-600 border border-green-100' : 'bg-red-50 text-red-600 border border-red-100'}`}>
                        {isMapped ? <><CheckCircle2 size={14}/> Alignment Complete</> : <><AlertTriangle size={14}/> Mapping Required</>}
                    </div>

                    <div className="flex gap-2 w-full sm:w-auto">
                        <Link href="/syllabus-generator/step-1" className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 sm:px-6 py-2.5 bg-slate-100 text-slate-700 rounded-xl font-bold hover:bg-slate-200 text-xs sm:text-sm border border-slate-200 transition-all">
                            <ChevronLeft size={16} /> Back
                        </Link>
                        <button onClick={validateAndNext} className="flex-2 sm:flex-none flex items-center justify-center gap-2 px-4 sm:px-8 py-2.5 bg-[#800000] text-white rounded-xl font-bold hover:bg-[#600000] text-xs sm:text-sm shadow-md active:scale-95 transition-all">
